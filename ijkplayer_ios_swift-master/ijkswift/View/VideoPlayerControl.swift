...
class VideoPlayerControl: UIControl {
    ...
    let transformBtn = ControlModel.sharedInstance.transformScreenBtn
    ...
    func addActtion() {
        ...
        transformBtn.addTarget(self, action: #selector(transformBtnAction(sender:)), for: UIControlEvents.touchUpInside)
    }
    ...
    func addGesture(view: VideoPlayerView) {
        ...
        GestureControlModel.sharedInstance.doubleTaped = { (gesModel) in
            self.transformBtnAction(sender: ControlModel.sharedInstance.transformScreenBtn)
        }
    }
    
    @objc func transformBtnAction(sender : UIButton) {
        sender.isSelected = !sender.isSelected
        if sender.isSelected {
            appDelegate.isAllOrientation = true
            sender.setTitle("还原", for: .normal)
