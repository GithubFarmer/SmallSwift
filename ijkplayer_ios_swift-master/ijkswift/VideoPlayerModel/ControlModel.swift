...
struct ControlModel {
    ...
    //强制转换屏幕
    let transformScreenBtn = UIButton.init()
    ...
    func initUI() {
        ...
        transformScreenBtn.setTitle("全屏", for: .normal)
    }
    ...
    func layoutUI(view: UIView) {
        ...
        bottomPanel.addSubview(transformScreenBtn)
        ...
        totalDurationLabel.snp.makeConstraints { (make) in
            make.top.bottom.equalTo(bottomPanel)
            make.right.equalTo(bottomPanel).offset(-50)
            make.size.width.equalTo(80)
        }
        slider.snp.makeConstraints { (make) in
            make.top.bottom.equalTo(bottomPanel)
            make.left.equalTo(bottomPanel).offset(150)
            make.right.equalTo(bottomPanel).offset(-125)
        }
        transformScreenBtn.snp.makeConstraints { (make) in
